...
public class AmenityAdapter extends BaseAdapter {
    ...
    private ArrayList<String> mArrSelected;

    public AmenityAdapter(Context mContext, ArrayList<Category> mArr, ArrayList<String> arrSelected) {
        this.mContext = mContext;
        this.mArr = mArr;
        mArrSelected = arrSelected;
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        for(int i = 0; i < mArrSelected.size(); i++){
            if(viewHolder.txvItem.getText().equals(mArrSelected.get(i))){
                viewHolder.imvSelected.setVisibility(View.VISIBLE);
                mArrSelected.remove(i);
            }
        }
